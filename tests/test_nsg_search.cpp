...
int main(int argc, char** argv) {
  ...
  // std::cout << "metric: " << index.distance_ << std::endl;
  // float temp1[1] = {1};
  // float temp2[1] = {-1};
  // std::cout << "dist = " << index.distance_->compare(temp1, temp2, 1) << std::endl;
  ...
  if (omp) {
    for (unsigned j = 0; j < query_num; j += batch_size) {
      ...
      for (unsigned i = j; i < j + batch_size; i++) {
        index.Search(query_load + i * dim, data_load, L, paras, res[i].data());
      }
    ...
    for (unsigned j = 0; j < query_num; j += batch_size) {
      ...
      for (unsigned i = j; i < j + batch_size; i++) {
        ...
        // std::cout << std::endl;
        // double recall_10 = calculate_recall_per_query(res[i], gt[i], 10);
        // std::cout << recall_10 << " ";
      }
      std::cout << std::endl;
      auto e = std::chrono::high_resolution_clock::now();
